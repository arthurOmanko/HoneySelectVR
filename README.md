...
- If you using other VR tools as of now except for here, your game might work unproperly. Then reconsider removing other VR tools (${GameFolder}/Plugins/**.dll files etc.). For example, might have to remove VRGIN_NEO.dll or StudioNEOVR.dll.  
...
`<CameraFrontLightRotationToDefaultOnSceneLoad>` | 0 | On loading a scene on StudioNEO, whether making the rotation of default directional front lights attached at VRCamera set at default rotation or not (using saved lights rotation data). If you always prefer default lights rotation, use this option. [0 = using saved data, 1 = set at default rotation]
